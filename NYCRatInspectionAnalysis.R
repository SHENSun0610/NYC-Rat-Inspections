...
#add years and months to data frame
...
#row of ones to keep track of count for every unique year-month pairing
rodent$ones = 1

#get data for rat sightings in years 2011-2015
...
#count up total sightings by borough for each year
...
#format all character dates to Date
...
#plot all boroughs on one chart
gg = ggplot() + 
...
#get data for all inspections in years 2011-2015
rodentInspections = subset(rodent, 
                           as.numeric(as.character(year))>2010 & as.numeric(as.character(year))<2016)

#separate by borough for rat sightings
...
#separate by borough for total inspections
...
#count up rat sightings and total inspections by borough for each year
...
#join tables for each borough by year
...
#efficiency for a given month is the number of inspections 
...
#get subset of data for inspections that yield active rat signs
...
#append corresponding boroughs
temp = subset(activeRat, select=c("ZIP_CODE","BOROUGH"))
zipRat = unique(inner_join(zipRat,temp,by="ZIP_CODE") %>% group_by(ZIP_CODE)) #append boroughs

#print top ten "hot spot" zipcodes among all 5 boroughs
print(zipRat[1:10,])
...
topBefore = unique(inner_join(topBefore,tempBefore,by="ZIP_CODE") %>% group_by(ZIP_CODE)) #append boroughs
...
topAfter = unique(inner_join(topAfter,tempAfter,by="ZIP_CODE") %>% group_by(ZIP_CODE)) #append boroughs
print(topAfter[1:20,])
...
topSandy = unique(inner_join(topSandy,tempSandy,by="Incident.Zip") %>% group_by(Incident.Zip)) #append boroughs
print(topSandy[1:20,])
...
#get rat sightings and total inspections
...
#outer join two tables and compute likelihood
...
#activeRatSightings is predictor variable: the likelihood of active rat signs
#in the same zip code as the restaurant, and in the same month and year 
#in which the restaurant was inspected


#STEP 2
#merge likelihood with restaurant violation data
...
#convert all NA to 0 values for activeRatSightings
